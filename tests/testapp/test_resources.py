...
class ResourceTest(TestCase):
    def test_list_view(self):
        ...
        self.assertContains(self.client.get("/resources/?page=2"), 'name="batch_', 2)
        # Invalid page number -> first page
        self.assertContains(self.client.get("/resources/?page=abc"), 'name="batch_', 5)
        # Empty page -> last page
        self.assertContains(self.client.get("/resources/?page=42"), 'name="batch_', 2)

        self.assertContains(self.client.get(r.get_absolute_url()), "Resource 6")
    ...
    def test_crud(self):
        ...
        self.assertRedirects(self.client.post(resource.urls["delete"]), "/resources/")
        self.assertEqual(Resource.objects.count(), 0)

    def test_modelviewurls(self):
        resource = Resource.objects.create()

        self.assertEqual(resource.urls["detail"], "/resources/%s/" % resource.pk)
        self.assertEqual(resource.urls["edit"], "/resources/%s/edit/" % resource.pk)
        self.assertEqual(resource.urls["delete"], "/resources/%s/delete/" % resource.pk)
        self.assertEqual(resource.urls["list"], "/resources/")
