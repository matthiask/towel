...
class ModelViewTest(TestCase):
    def test_list_view(self):
        ...
        # paginate_by=5
        self.assertContains(self.client.get("/persons/"), 'name="batch_', 5)
        self.assertContains(self.client.get("/persons/?page=2"), 'name="batch_', 2)
        # Invalid page number -> first page
        self.assertContains(self.client.get("/persons/?page=abc"), 'name="batch_', 5)
        # Empty page -> last page
        self.assertContains(self.client.get("/persons/?page=42"), 'name="batch_', 2)
        # Everything
        self.assertContains(self.client.get("/persons/?all=1"), 'name="batch_', 7)

        self.assertContains(self.client.get(p.get_absolute_url()), "Family 6")
    ...
    def test_crud(self):
        ...
        self.assertContains(self.client.get(person.get_absolute_url()), "Blab Blub")

        self.assertContains(self.client.get(person.urls["edit"]), "<form", 1)
        response = self.client.post(
            person.urls["edit"],
            {
                "family_name": "Blub",
        ...
        self.assertRedirects(self.client.post(person.urls["delete"]), "/persons/")
    ...
    def test_crud_formsets(self):
        ...
        self.assertContains(self.client.get(person.urls["detail"]), "Blab Blubbber")

    def test_modelviewurls(self):
        person = Person.objects.create()

        self.assertEqual(person.urls["detail"], "/persons/%s/" % person.pk)
        self.assertEqual(person.urls["edit"], "/persons/%s/edit/" % person.pk)
        self.assertEqual(person.urls["delete"], "/persons/%s/delete/" % person.pk)
        self.assertEqual(person.urls["list"], "/persons/")
    ...
    def test_emailaddress_views(self):
        ...
        self.assertContains(
            response,
            '<a href="/emailaddresses/%s/">test@example.com</a>' % (emailaddress.pk),
        )
        self.assertContains(
            response, '<a href="/persons/%s/">Testa Testi</a>' % emailaddress.person_id
        )
    ...
    def test_automatic_get_absolute_url(self):
        ...
        message = Message.objects.create(
            sent_to=EmailAddress.objects.create(person=Person.objects.create()),
        )

        self.assertEqual(message.get_absolute_url(), "/messages/%s/" % message.pk)
