...
class UtilsTest(TestCase):
    ...
    def test_safe_queryset_and(self):
        ...
        qs1 = (
            EmailAddress.objects.search("blub")
            .transform(_transform_nothing)
            .select_related()
        )
        qs2 = EmailAddress.objects.distinct().reverse().select_related("person")
    ...
    def test_template_tag_helpers(self):
        testcases = [
            ...
            ('{% testtag bla="blub" blo="blob" %}', "ARGS: KWARGS: bla=blub,blo=blob"),
            ('{% testtag bla=blub blo="blob" %}', "ARGS: KWARGS: bla=blubber,blo=blob"),
        ]
